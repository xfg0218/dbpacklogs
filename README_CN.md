...
| **安全加固** | 通过 stdin 传递路径防止 SSH 命令注入 |
| **时区支持** | TimeFilter 支持 Location 时区参数，精确过滤日志 |
| **改进的错误处理** | 更清晰的错误信息和更好的故障隔离 |
...
**Q3.1：时区处理是如何工作的？**
TimeFilter 现在支持 Location 时区参数。所有时间解析和过滤操作都尊重配置的时区，确保跨地区部署时的日志过滤准确性。
...
**Q8：做了哪些安全改进？**
工具现在通过 stdin 将文件路径传递给远程 SSH 命令，而不是嵌入到命令字符串中。这防止了潜在的命令注入攻击。此外，所有用户输入在使用前都会经过 shell 危险字符验证。
